        ...
        if id and ammo then
            ...
            if model then
                ...
                lua_thread.create(function()
                    ...
                    sampAddChatMessage("{00FF00}[Gun-Cheat]{FFFFFF} Âû âûäàëè ñåáå îðóæèå ID: " .. id, -1)
                end)
            else
                sampAddChatMessage("{FF0000}[Îøèáêà]{FFFFFF} Íåâåðíûé ID èëè ìîäåëü îðóæèÿ íå íàéäåíà.", -1)
            end
        else
            sampAddChatMessage("{FFFF00}[Ïîäñêàçêà]{FFFFFF} Èñïîëüçóéòå: /getgun [ID] [Ammo]", -1)
        end
    end)
